...
% ── Title block ─────────────────────────────────────────────────────────────
\title{%
  \textbf{\malariai: A Label-Resilient Decoupled Framework for\\ Universal Cell Segmentation and Explainable Stage\\
  Classification in Dense Malaria Blood Smears}%
}

\author[1]{Kaysarul Anas Apurba\thanks{Corresponding author: \{kapurba\}@laurentaian.ca}}
\author[2]{Mohammdel Ali\thanks{Melbourne Institute of Technology, Australia}}
...
a critical open problem in global health AI, particularly in resource-limited settings where expert microscopists are scarce. Despite significant progress in applying deep learning to this domain, three compounding failure modes persist across the literature and remain unresolved in any single published framework. First, end-to-end detectors (\eg, Faster R-CNN, YOLO) treat unannotated cells as background, systematically suppressing true positives in sparsely labelled datasets --- a documented consequence of incomplete ground-truth that no prior decoupled architecture has directly addressed on a large public benchmark. Second, Non-Maximum Suppression (NMS) discards valid cell detections in high-density smear regions where red blood cells routinely overlap, causing systematic under-counting that is clinically dangerous. Third, existing pipelines produce opaque class labels without spatial evidence, severely limiting clinical adoption and physician trust. We present \malariai{}, a two-stage framework that addresses all three limitations in a unified pipeline. In Stage~1, an annotation-agnostic distance-transform guided watershed algorithm isolates every cell in the blood smear regardless of whether a ground-truth label exists, achieving label-resilient cell
...
on each extracted crop, with Grad-CAM\texttt{++} generating per-cell spatial attention heatmaps as an integrated clinical explainability layer.
We benchmark \malariai{} against a Faster R-CNN baseline on the publicly
available NIH BBBC041 dataset comprising 1,208 training images and 79,672
valid annotated instances across six cell categories, and demonstrate measurable
...
to the incomplete-annotation problem inherent in real-world malaria datasets, and that integrated explainability is achievable without sacrificing detection performance.
\end{abstract}
...
architecturally elaborate system in this category, combining a
...
Parameter Fennec Fox Optimisation (URP-FFO) metaheuristic algorithm.
Crucially, however, this framework is evaluated exclusively on
Kaggle-hosted pre-cropped cell image datasets -- the Malaria Cell
Images Dataset (27{,}558 patches) and a secondary Malaria Detection
Dataset -- and performs \emph{binary} classification only (parasitised
vs.\ uninfected erythrocyte). It does not operate on whole blood smear
images, does not perform multi-class parasite stage detection (ring,
trophozoite, schizont, gametocyte), and does not use the NIH BBBC041
benchmark. The reported best result of 94.88\% accuracy (Dataset~1)
is therefore a binary patch-level metric and is not directly comparable
to object detection mAP evaluated on full-slide images. This places
the work squarely in the pre-isolated cell classification family
(\S\ref{ssec:cnn_class}) despite its architectural framing as a
detection pipeline.

T-MUNet\texttt{++} integrates a Vision Transformer attention layer at
the bottleneck with the nested dense skip connections of
UNet\texttt{++}~\cite{zhou2018unetpp} and depthwise separable
convolutions from MobileNet; atrous (dilated) convolution in AA-CRMV2
provides multi-scale receptive fields; and recurrent connections enable
iterative feature refinement. Despite this sophistication, three
critical gaps remain. The URP-FFO metaheuristic introduces significant
reproducibility concerns -- metaheuristic methods applied to neural
network training are sensitive to random initialisation and offer no
convergence guarantees, making independent replication impractical.
No class imbalance mechanism is present. No spatial explainability
layer is integrated. And because no mAP metric is reported at any IoU
threshold, quantitative comparison with whole-slide detection baselines
is impossible.
...
blood smear images acquired under a $100\times$ oil-immersion objective.
The dataset comprises 1,208 training images and 120 held-out test images,
each of uniform resolution $1{,}600 \times 1{,}200$ pixels (RGB). Raw
annotations total 80,113 bounding boxes across the training set and 5,922
across the test set. Each bounding box carries one of seven category labels:
\textit{red blood cell}, \textit{trophozoite}, \textit{ring},
\textit{schizont}, \textit{gametocyte}, \textit{leukocyte}, and
\textit{difficult}. Following standard practice, \textit{difficult}
annotations (441 in training; 5 in test) are excluded from all evaluations,
yielding 79,672 and 5,917 valid boxes respectively.

\paragraph{Class distribution and imbalance.}
Table~\ref{tab:class_dist} and Figure~\ref{fig:class_dist} report the
per-class annotation counts across both splits. The distribution is
severely imbalanced: red blood cells constitute 97.2\% of all training
annotations, while the rarest parasitic stage (gametocyte) accounts for
only 0.18\%, a ratio of 537:1. This is not a mild statistical skew -- it
is a qualitative imbalance that renders standard cross-entropy loss
ineffective for the minority classes: a model predicting \textit{red blood
cell} for every instance achieves 97.2\% training accuracy while detecting
zero parasites. This motivates the use of Focal Loss~\cite{lin2017focal}
with per-class inverse-frequency weights in both pipelines
(Section~\ref{ssec:stage2}).

\begin{table}[h!]
\centering
\small
\caption{Per-class annotation counts in NIH BBBC041.
         \textit{Difficult} annotations are excluded.}
\label{tab:class_dist}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lrrl}
\toprule
\textbf{Class} & \textbf{Train} & \textbf{Test} & \textbf{\%} \\
\midrule
Red blood cell  & 77{,}420 & 5{,}614 & 97.2\% \\
Trophozoite     &  1{,}473 &    111  &  1.8\% \\
Ring            &    353   &    169  &  0.4\% \\
Schizont        &    179   &     11  &  0.2\% \\
Gametocyte      &    144   &     12  &  0.2\% \\
Leukocyte       &    103   &      0  &  0.1\% \\
\midrule
\textbf{Total}  & \textbf{79{,}672} & \textbf{5{,}917} & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h!]
  ...
  \caption{Class distribution of NIH BBBC041 training annotations.
    \textit{Left:} log-scale view showing the dominance of red blood cells
    (77,420) over all parasitic stages combined (2,252). \textit{Right:}
    linear-scale view of parasitic stages only, revealing the secondary
    imbalance between trophozoites (1,473) and the rarest stages
    (gametocyte: 144). Both pipelines address this using Focal Loss with
    per-class inverse-frequency weights.}
  \label{fig:class_dist}
\end{figure}

\paragraph{Parasitic stage morphology.}
Figure~\ref{fig:parasite_crops} shows representative crops of each
parasitic stage extracted from the training set. The visual similarity
between ring-stage infections and healthy RBCs is immediately apparent:
a ring-stage erythrocyte presents as a pale disc with only a faint
peripheral chromatin outline, closely resembling an uninfected cell.
Trophozoites exhibit irregular, scattered chromatin mass; schizonts show
clear internal segmentation (merozoites); gametocytes appear as large,
densely stained bodies. This morphological hierarchy directly predicts
which classes EfficientNet will struggle with most -- ring stages are
expected to yield the lowest per-class AP -- and motivates reporting
per-class AP in addition to aggregate mAP.
...
\begin{figure}[h!]
  ...
  \caption{Representative ground-truth crops of each parasitic stage in
    NIH BBBC041, shown at native resolution with bounding box overlay.
    Crop dimensions in pixels are indicated above each cell. Ring-stage
    infections are visually the most similar to healthy RBCs, suggesting
    they will be the most challenging class for the Stage~2 classifier.
    Parasitic stages are systematically larger than healthy erythrocytes
    (trophozoite median area: 17{,}272\,px$^2$; RBC: 11{,}544\,px$^2$),
...
Figure~\ref{fig:density} shows the distribution of annotated bounding
boxes per image across the training set. Annotation density ranges from
9 to 223 boxes per image, with a median of 59 and a 95th-percentile of
137. Eleven percent of images contain more than 100 annotated cells.
To quantify the overlap problem (P2), we computed pairwise IoU between
all ground-truth boxes within each image across a 200-image sample.
\textbf{58\%} of images contain at least one ground-truth pair with
$\text{IoU} > 0.3$, confirming that cell overlap is a structural feature
of the data rather than an edge case. This is the empirical motivation
for distance-transform watershed as Stage~1: by separating cells at the
morphological level before any proposal-scoring or NMS step, the pipeline
avoids the systematic suppression that affects all anchor-based detectors
in dense regions (Section~\ref{ssec:stage1}).

\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{fig_density.png}
  \caption{Distribution of annotated bounding boxes per image in the
    NIH BBBC041 training set (1,208 images). The dashed red line marks
    the median (59 boxes). The dotted orange line at 100 boxes identifies
    images we classify as \emph{dense smears} for the purpose of our
    dense-region recall evaluation. 11\% of images exceed this threshold;
    58\% of a 200-image sample contain at least one overlapping cell pair
    ($\text{IoU} > 0.3$), demonstrating the severity of the NMS failure
    mode in this dataset.}
  \label{fig:density}
\end{figure}

\paragraph{Train/validation/test protocol.}
The 1,208 training images are partitioned 80/20 at the \emph{image level}
into a training split (966 images) and a validation split (242 images),
ensuring that no image contributes boxes to both sets. The random seed is
fixed at 42 for reproducibility. All hyperparameter selection and early
stopping decisions use the validation split exclusively. The 120 test images
are held out and evaluated only once per pipeline, at the conclusion of
training.

\subsection{Stage 1: Universal Cell Segmentation}
\label{ssec:stage1}
\placeholder{[To be completed. Key content: Otsu thresholding, morphological opening, distance transform mathematics, watershed algorithm, AREA\_MIN/AREA\_MAX parameter justification from EDA, N1 and N2 claims formalised.]}

\subsection{Stage 2: EfficientNet-B0 Classifier with Focal Loss}
\label{ssec:stage2}
\placeholder{[To be completed. Key content: EfficientNet compound scaling, crop extraction with 6px margin, Focal Loss derivation and gamma=2.0 justification, per-class alpha weights from EDA counts, training augmentation (ColorJitter for stain variation).]}

\subsection{Grad-CAM\texttt{++} Explainability}
\label{ssec:gradcam}
\placeholder{[To be completed. Key content: Grad-CAM++ formula, integration point in pipeline (applied to EfficientNet final conv), output as per-cell heatmap over original image crop, N3 claim formalised.]}

\subsection{Baseline A: Faster R-CNN}
\label{ssec:baseline}

Pipeline A serves as our primary quantitative baseline and is implemented as
a Faster R-CNN~\cite{ren2015} with a ResNet-50 Feature Pyramid Network (FPN)
backbone pretrained on MS-COCO. The standard 91-class classification head is
replaced by a \texttt{FastRCNNPredictor} with 7 output classes (background
plus the six BBBC041 categories). The FPN generates multi-scale feature maps
at strides \{4, 8, 16, 32, 64\}, enabling simultaneous detection of large
cells (\eg, leukocytes, median area $\approx$14,000\,px$^2$) and small
ring-stage nuclei ($\approx$3,500\,px$^2$).

\paragraph{Training protocol.}
The model is trained for 80 epochs on the 966-image training split using
Stochastic Gradient Descent (SGD) with initial learning rate $5 \times
10^{-3}$, momentum 0.9, and weight decay $10^{-4}$. A MultiStepLR schedule
reduces the learning rate by a factor of 10 at global epoch 50.
Gradient clipping with maximum norm 10.0 prevents loss spikes from
high-density images. Training uses batch size 4 on an NVIDIA T4 GPU
(16\,GB VRAM). The best checkpoint is selected by minimum validation loss.

\paragraph{Structural training ceiling.}
Figure~\ref{fig:frcnn_loss} shows the training and validation loss curves
over 80 epochs. Three observations characterise the baseline's fundamental
limitation. First, validation loss plateaus at approximately 0.23 from
epoch~8 while training loss continues to decrease, reaching 0.11 by
epoch~50. This divergence is not conventional overfitting: the model is
correctly detecting unannotated cells that genuinely exist in the image,
but the closed-world loss function penalises these detections as false
positives. Second, after the learning rate drop at epoch~50, training
loss decreases further to 0.08 but validation loss \emph{increases} to 0.28.
Smaller gradient updates cause the model to commit more aggressively to
detections of unannotated cells, compounding the annotation artefact.
Third, the best checkpoint is selected at epoch~23 (val.\ loss 0.2294):
the 57 subsequent training epochs yield no improvement and in fact degrade
validation performance. These observations directly confirm the P1 failure
mode -- the validation ceiling is not an optimisation failure but a
structural consequence of annotation incompleteness that no learning rate
schedule, data augmentation, or additional training can resolve within
the end-to-end detection paradigm.

\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{fig_frcnn_loss.png}
  \caption{Faster R-CNN (Baseline A) training and validation loss over
    80 epochs on NIH BBBC041. Validation loss plateaus at $\approx 0.23$
    from epoch~8 while training loss continues to fall. After the learning
    rate drop at epoch~50, validation loss \emph{increases} to 0.28 as the
    model over-commits to detections on unannotated cells -- a direct
    manifestation of Problem~P1. Best checkpoint selected at epoch~23
    (val.\ loss 0.2294).}
  \label{fig:frcnn_loss}
\end{figure}

\paragraph{Qualitative predictions.}
Figure~\ref{fig:frcnn_predictions} shows ground-truth annotations (top row)
versus Faster R-CNN predictions (bottom row) on four held-out validation
images. The model successfully detects red blood cells at high precision
and recalls leukocytes consistently given their distinctive size and staining.
Parasite-stage detections are visible but sparse, particularly for
gametocyte and schizont. In the rightmost column, multiple genuinely distinct
cells in a dense region receive a single fused detection box or are
suppressed by NMS, directly illustrating the P2 failure mode.

\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{fig_frcnn_predictions.png}
  \caption{Ground truth (top) versus Faster R-CNN predictions (bottom)
    on four validation images. Colour coding follows the class legend:
    red~= red blood cell; remaining colours~= parasitic stages and
    leukocyte. NMS suppression of overlapping cells in dense-region
    images (P2) is visible in the rightmost column.}
...
All detection results are reported as mean Average Precision at IoU
threshold 0.5 (mAP@0.5), computed using the \texttt{torchmetrics}
\texttt{MeanAveragePrecision} implementation with \texttt{class\_metrics=True}
to produce per-class AP values alongside the aggregate. A predicted box is
counted as a true positive if its IoU with a ground-truth box of the same
class exceeds 0.5; detections with confidence below 0.3 are discarded
before evaluation.

Per-class AP is reported for all six BBBC041 categories separately, as the
aggregate mAP is dominated by the red blood cell class (97.2\% of
annotations) and obscures performance on the clinically critical parasitic
stages. We specifically highlight schizont and gametocyte AP as key
indicators of rare-class performance, since these are the classes most
affected by the 537:1 class imbalance.

For Stage~1 (watershed segmentation), we report cell recovery rate -- the
fraction of ground-truth bounding boxes that contain at least one
watershed-detected region -- on a 50-image held-out evaluation set.
A watershed region is considered a match if its centroid falls within
the ground-truth box boundary.

Dense-region recall is evaluated separately on images with more than 100
annotated boxes (11\% of training images), to isolate the P2 failure
mode from aggregate metrics.
...
\paragraph{Hardware.}
Baseline A (Faster R-CNN) training was conducted on an NVIDIA T4 GPU
(16\,GB VRAM) via Kaggle Notebooks. Pipeline B (EfficientNet-B0) training
is conducted on the same hardware. Inference for the deployed web application
(Phase~4) runs entirely on CPU; EfficientNet-B0 processes one blood smear
in approximately 2 seconds on a standard compute instance.

\paragraph{Baseline A (Faster R-CNN).}
Architecture: ResNet-50 FPN backbone (MS-COCO pretrained), 7-class
\texttt{FastRCNNPredictor} head. Optimiser: SGD ($lr = 5 \times 10^{-3}$,
momentum $= 0.9$, weight decay $= 10^{-4}$). Schedule: MultiStepLR,
$\gamma = 0.1$ at epoch~50. Batch size: 4. Epochs: 80. Gradient clipping:
max norm 10.0. Best checkpoint: epoch~23 (val.\ loss 0.2294). Training
time: $\approx$5.5 hours on T4.

\paragraph{Pipeline B (MalariAI).}
\placeholder{[To be completed after Phase 3 training. Key content:
EfficientNet-B0 hyperparameters, Focal Loss $\gamma$ and per-class $\alpha$,
crop size, augmentation strategy, number of epochs.]}

\subsection{Quantitative Results}
\label{ssec:quant}

Table~\ref{tab:results} reports per-class AP@0.5 and mAP@0.5 for both
pipelines on the 242-image validation split. Baseline~A results are final;
Pipeline~B results will be inserted upon completion of Phase~3 training.

\begin{table}[h!]
\centering
\small
\caption{Per-class AP@0.5 and mAP@0.5 on the NIH BBBC041 validation split
  (242 images, seed 42). Classes sorted by decreasing Baseline~A AP.
  Pipeline~B results pending.
  $\dagger$~= rare class ($<200$ training instances).}
\label{tab:results}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lcc}
\toprule
\textbf{Class} & \textbf{Baseline A} & \textbf{MalariAI (B)} \\
               & Faster R-CNN        & Watershed + EffNet-B0 \\
\midrule
Red blood cell  & 90.94\% & --- \\
Leukocyte       & 88.12\% & --- \\
Trophozoite     & 67.22\% & --- \\
Ring            & 57.17\% & --- \\
Gametocyte$\dagger$ & 25.95\% & --- \\
Schizont$\dagger$   & 24.57\% & --- \\
\midrule
\textbf{mAP@0.5} & \textbf{58.99\%} & \textbf{---} \\
\bottomrule
\end{tabular}
\end{table}

The per-class breakdown reveals the severity of class imbalance under the
end-to-end detection paradigm. Gametocyte (144 training instances, 0.18\%
of boxes) achieves only 25.95\% AP -- less than one quarter of the dominant
class performance. Schizont (179 instances) achieves 24.57\%. Both rare
classes fall below 26\% AP despite the model training for 80 epochs with
COCO-pretrained weights. Trophozoite (1,473 instances) achieves 67.22\%,
demonstrating that even a modest increase in training examples produces
a commensurate improvement in detection quality. Ring stage (57.17\%) is
the weakest non-rare class, consistent with its visual similarity to
healthy RBCs noted in Section~\ref{ssec:dataset}. These baselines
establish the minimum target that Pipeline~B must surpass on each class
to validate the contribution of Focal Loss and the decoupled architecture.

\subsection{Ablation Study}
\label{ssec:ablation}
\placeholder{[To be completed after Phase 3. Key comparisons:
(i) watershed alone vs.\ Faster R-CNN for cell recovery rate;
(ii) EfficientNet-B0 with cross-entropy vs.\ Focal Loss;
(iii) Pipeline B with vs.\ without Grad-CAM++ (inference speed overhead).]}

\subsection{Dense Region Analysis}
\label{ssec:dense}
\placeholder{[To be completed. Metric: recall on images with $>$100
annotated boxes. Expected result: watershed recovers cells that NMS
suppresses in dense regions, producing higher recall in the
$>$100-box subgroup.]}

\subsection{Qualitative Explainability Results}
\label{ssec:xai_results}
\placeholder{[To be completed after Phase 3. Content: Grad-CAM++ heatmaps
on representative crops from each parasitic stage, dual view (crop
detail and full-image overlay), comparison of correct vs.\ misclassified
cells to show what the model attends to in ambiguous cases.}]
...
The authors would like to thank their former supervisor from North South University, Dhaka, Bangladesh, currently working as an Assistant Professor at King Fahd University of Petroleum and Minerals (KFUPM), for his guidance during the Junior Design 299 course in 2021, where the initial concept of this work was first developed.

The authors also gratefully acknowledge Prof.\ Amr Abdel-Dayem (Laurentian University, Canada) for his invaluable guidance during the Image Processing and Computer Vision course within the M.Sc.\ programme in Computational Sciences at Laurentian University. This project was revisited and significantly extended as a course project in Fall 2023 under his supervision, forming the direct foundation of the present work.
...
\newblock NIH BBBC041 dataset: \url{https://bbbc.broadinstitute.org/BBBC041}.
...
\newblock A robust malaria cell detection framework using adaptive and atrous
  convolution-based recurrent {MobileNetV2} with {Trans-MobileUNet\texttt{++}}-based
